...
import (
	...
	dialectpkg "github.com/aircast-one/gomavlib/v4/pkg/dialects/{{ .PkgName }}"
)
...
func TestEnum_{{ .Name }}(t *testing.T) {
	...
	t.Run("first entry", func(t *testing.T) {
		enc, err := dialectpkg.{{ .FirstEntry }}.MarshalText()
		require.NoError(t, err)

		var dec dialectpkg.{{ .Name }}
		err = dec.UnmarshalText(enc)
		require.NoError(t, err)

		require.Equal(t, dialectpkg.{{ .FirstEntry }}, dec)
	})
}
`))
