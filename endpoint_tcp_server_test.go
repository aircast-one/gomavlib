...
func TestEndpointTCPServer(t *testing.T) {
	node := &Node{
		Dialect:          testDialect,
		OutVersion:       V2,
		OutSystemID:      10,
		Endpoints:        []Endpoint{&EndpointTCPServer{Address: "127.0.0.1:5605"}},
	...
	conn, err := net.Dial("tcp", "127.0.0.1:5605")
	require.NoError(t, err)
	defer conn.Close()
...
func TestEndpointTCPServerIdleTimeout(t *testing.T) {
	node := &Node{
		Dialect:          testDialect,
		OutVersion:       V2,
		OutSystemID:      10,
		Endpoints:        []Endpoint{&EndpointTCPServer{Address: "127.0.0.1:5606"}},
		HeartbeatDisable: true,
	...
	conn, err := net.Dial("tcp", "127.0.0.1:5606")
	require.NoError(t, err)
	defer conn.Close()
